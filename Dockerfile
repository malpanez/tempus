# Build stage
# Pins golang:1.26-alpine. Digest resolved 2026-09-07 from the multi-arch
# manifest index (serves linux/amd64 and linux/arm64, both required by
# release.yml). Digest-only (no tag) per SonarCloud S6596 — a tag+digest
# combo is flagged as redundant since the digest alone is what's pulled.
...
# Final stage
# Pins alpine:3.21. Digest resolved 2026-09-07 from the multi-arch manifest
# index (serves linux/amd64 and linux/arm64, both required by release.yml).
# Digest-only (no tag) per SonarCloud S6596.
